function [] = RevisedFittingContactDataAzeroMassActVsepIFNalpha
close all
% turn figure display on  or off
set(0,'DefaultFigureVisible','on')
% import and organize data
xs = readtable('IFNaDataContact.csv');
...
array2table(pfit)
   ...
   %pfitsContactFinal(j,:) = pfit;   %lambda1=(1/11.8)*pfit(end-2);
    ...
    x0 = [pfit(5),vv(1),pfit(8)*pfit(5)]; % initial conditions 
    atilde=0; v0 = pfit(7);
  f = @(t,a) [lambda+k*(a(2)/(v0+a(2)))*a(1)- d*a(1); ... 
               (r*(1-(a(2)/K))-theta*a(1)-delta*a(3))*a(2); ...
                (atilde*(a(1)/(1+a(1)))+b*a(3))*a(2)];
...
%    pvfit = fits(2*length(tt)+1:end);
    ...
    str2= strcat('ID',num2str(IDs(j)),groups(j),'A_0','IFNa');
%    length(tt(1:end-1))
%    length(xfit)
   figure
   hold on
...
save('pfitsContactFinalIFNa.mat','pfitsContactFinal')
save('maxViralContactFinalIFNa.mat','maxViralContact')
save('cumviralContactFinalIFNa.mat','cumviralContact')
%save('VTCFinal.mat','VTC')
save('xfitsCFinalIFNa.mat','xfitsC')
save('sfitsCFinalIFNa.mat','sfitsC')
